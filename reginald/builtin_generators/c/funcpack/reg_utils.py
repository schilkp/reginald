...
def generate(rmap: RegisterMap, name: NameGenerator, output_file: str, opt):
    ...
    for block_name, block in rmap.registers.items():
        for template_name, template in block.registers.items():
            if len(template.fields) == 0:
                ...
                continue
            struct_name = name.reg_struct_name(block_name, template_name)
            modify_func = name.reg_modify_func(block_name, template_name)
    ...
    for block_name, block in rmap.registers.items():
        for template_name, template in block.registers.items():
            if len(template.fields) == 0:
                ...
                continue
            struct_name = name.reg_struct_name(block_name, template_name)
            pack_func = name.reg_pack_func(block_name, template_name)
    ...
    for block_name, block in rmap.registers.items():
        for template_name, template in block.registers.items():
            if len(template.fields) == 0:
                ...
                continue
            struct_name = name.reg_struct_name(block_name, template_name)
            unpack_func = name.reg_unpack_func(block_name, template_name)
    ...
    for block_name, block in rmap.registers.items():
        for template_name, template in block.registers.items():
            ...
            out.append(f"/**")
            out.append(
                f" * \\defgroup {name.doxygroup_regfuncs(block_name, template_name)} {block_name+template_name} register modify/pack/unpack utilities.")
            out.append(f" * @{{")
            out.append(f" */")

            packed_type = name.reg_packed_type(template)
            struct_name = name.reg_struct_name(block_name, template_name)
            modify_func = name.reg_modify_func(block_name, template_name)
            pack_func = name.reg_pack_func(block_name, template_name)
            unpack_func = name.reg_unpack_func(block_name, template_name)
            unpack_macro = name.reg_unpack_macro(block_name, template_name)

            if opt.short_packfunc_comment:
                out.append(f"/** @brief Modify the '{block_name+template_name}' register's binary representation */")
            else:
                out.append(f"/**")
                out.append(f" * @brief Modify the '{block_name+template_name}' register's binary representation")
                out.append(f" * All fields are replaced with the struct's values.")
            ...
            if template.always_write is not None:
                out.append(f"  val &= ~{name.reg_alwayswrite_mask_macro(block_name, template_name)};")
                out.append(f"  val |= {name.reg_alwayswrite_val_macro(block_name, template_name)};")
            ...
            if opt.short_packfunc_comment:
                out.append(f"/** @brief Pack the '{block_name+template_name}' register's fields into their binary representation. */")
            else:
                out.append(f"/**")
                out.append(f" * @brief Pack the '{block_name+template_name}' register's fields into their binary representation")
                out.append(f" * All fields are set to the struct's values.")
            ...
            if opt.short_packfunc_comment:
                out.append(f"/** @brief Unpack the '{block_name+template_name}' register's binary representation into seperate fields. */")
            else:
                out.append(f"/**")
                out.append(f" * @brief Unpack the '{block_name+template_name}' register's binary representation into seperate fields")
            ...
            for field in template.fields.values():
                member_name = name.reg_struct_member(field)
                member_type = name.reg_struct_member_type(block_name, template_name, field)
                mask = field.bits.get_bitmask()
                shift = field.bits.lsb_position()
            ...
            out.append(f"/**")
            out.append(f" * @brief Unpack the '{block_name+template_name}' register's binary representation into a struct initialiser.")
