...
use derive::{
    derive_enum_from_bytes, derive_enum_to_bytes, derive_enum_try_from_bytes, derive_enum_wrapping_from_bytes,
...
#[proc_macro_derive(WrappingFromBytes, attributes(reginald))]
pub fn derive_wrapping_from_bytes(inp: TokenStream) -> TokenStream {
    let inp = parse_macro_input!(inp as input::ReginaldDeriveInput);
    let result = match inp {
        ...
        input::ReginaldDeriveInput::Enum(enum_data) => derive_enum_wrapping_from_bytes(&enum_data),
    };
    match result {
        Ok(s) => s.into(),
        Err(e) => e.into_compile_error().into(),
